...
class QSR_TPCC(QSR_Triadic_1t_Abstractclass):
    ...
    def _compute_qsr(self, origin, relatum, objct, qsr_params, **kwargs):
        ...
        :param origin: <qsrlib_io.world_trace.Object_State object> o1
        :param relatum: <qsrlib_io.world_trace.Object_State object> o2
        :param objct: <qsrlib_io.world_trace.Object_State object> o3
        :param qsr_params:
        :param kwargs:
        :return:
        """
        base_distance = math.sqrt((origin.x-relatum.x)**2 + (origin.y-relatum.y)**2) # distance between o1 and o2
        object_distance  = math.sqrt((objct.x-relatum.x)**2 + (objct.y-relatum.y)**2) # distance between o3 and o2
        if object_distance == 0:
            return "sam"  #same location
