...
namespace VoxSimPlatform {
    namespace Vox {
        ...
        //		voxml = VoxML.LoadFromText (www.text);
        		...
        		minYBound = Global.Helper.GetObjectWorldSize(gameObject).min.y;
        		//Debug.Log (minYBound);
        ...
        //		turnSpeed = defaultTurnSpeed;
        		...
        		Debug.Log(Global.Helper.VectorToParsable(Global.Helper.GetObjectWorldSize(gameObject).min));
        		Debug.Log(Global.Helper.VectorToParsable(Global.Helper.GetObjectWorldSize(gameObject).max));
        	}
        		...
        		if (interTargetPositions.Count == 0) {
        			...
        			if (!isGrasped) {
        				...
        				if (offset.sqrMagnitude <= Constants.EPSILON) {
                            ...
                            Debug.Log(string.Format("Removing {0} from {1}.interTargetPositions", Global.Helper.VectorToParsable(interimTarget), gameObject.name));
        ...
        //							}
        		...
        		if (supportingSurface != null) {
        			...
        			Bounds surfaceBounds = Global.Helper.GetObjectWorldSize(supportingSurface);
        			Bounds objectBounds = Global.Helper.GetObjectWorldSize(gameObject);
        ...
        //			}
        			...
        			if (Global.Helper.IsTopmostVoxemeInHierarchy(gameObject)) {
        	...
        	void AdjustToSupportingSurface() {
        		Vector3 rayStartX = new Vector3(Global.Helper.GetObjectWorldSize(gameObject).min.x - Constants.EPSILON,
        			Global.Helper.GetObjectWorldSize(gameObject).min.y + Constants.EPSILON,
        			Global.Helper.GetObjectWorldSize(gameObject).center.z);
        		Vector3 contactPointX = Global.Helper.RayIntersectionPoint(rayStartX, Vector3.right);
        		//contactPointX = new Vector3 (contactPointX.x, transform.position.y, contactPointX.z);

        		Vector3 rayStartZ = new Vector3(Global.Helper.GetObjectWorldSize(gameObject).center.x,
        			Global.Helper.GetObjectWorldSize(gameObject).min.y + Constants.EPSILON,
        			Global.Helper.GetObjectWorldSize(gameObject).min.z - Constants.EPSILON);
        		Vector3 contactPointZ = Global.Helper.RayIntersectionPoint(rayStartZ, Vector3.forward);
        			...
        			if (hit.collider.gameObject.GetComponent<BoxCollider>() != null) {
        				if ((!hit.collider.gameObject.GetComponent<BoxCollider>().isTrigger) &&
        					...
        					if (!Global.Helper.FitsIn(Global.Helper.GetObjectWorldSize(hit.collider.gameObject),
        						...
        						//if (!grasped) {
        						bool themeIsConcave = (Global.Helper.GetMostImmediateParentVoxeme(gameObject)
                                ...
                                bool supportIsConcave = (Global.Helper.GetMostImmediateParentVoxeme(supportingSurface)
        							.GetComponent<Voxeme>().voxml.Type.Concavity.Contains("Concave"));
        						...
        						if ((themeIsConcave) &&
        						    (Concavity.IsEnabled(Global.Helper.GetMostImmediateParentVoxeme(gameObject))) &&
        						    (Global.Helper.FitsIn(Global.Helper.GetObjectWorldSize(supportingSurface.transform.root.gameObject),
        							    Global.Helper.GetObjectWorldSize(gameObject)))) {
        							minYBound = Global.Helper.GetObjectWorldSize(supportingSurface).min.y;
        							...
        							if (supportIsConcave) {
        								...
        								if (Concavity.IsEnabled(Global.Helper.GetMostImmediateParentVoxeme(supportingSurface))) {
        									...
        									//Debug.Break ();
        									minYBound = Global.Helper.GetObjectWorldSize(supportingSurface).max.y;
        									//								Debug.Log (minYBound);
        									//Debug.Log (minYBound);
        								}
        							}
        	...
        	public void Reset() {
        		if (gameObject.transform.parent != null) {
        			...
        			Voxeme parentVox = Global.Helper.GetMostImmediateParentVoxeme(parent).GetComponent<Voxeme>();
        	...
        	public Vector3 MoveToward(Vector3 target) {
        		Debug.Log (string.Format("{0}: at {1}, moving toward {2}",gameObject.name,
                    Global.Helper.VectorToParsable(transform.position),Global.Helper.VectorToParsable(target)));
        		if (!isGrasped) {
        			Vector3 offset = transform.position - target;
        			Vector3 normalizedOffset = Vector3.Normalize(offset);

                    Debug.Log (string.Format("{0}: {1}, {2}",gameObject.name,
                        Global.Helper.VectorToParsable(offset),Global.Helper.VectorToParsable(normalizedOffset)));
        	...
        	void OnTargetPositionChanged(Vector3 oldVal, Vector3 newVal) {
        		Debug.Log(string.Format("==================== Target position changed ==================== {0}: {1}->{2}",
        			gameObject.name, Global.Helper.VectorToParsable(oldVal), Global.Helper.VectorToParsable(newVal)));
        	}
        	...
        	void PopulateOperationalVoxeme() {
        		...
        		foreach (VoxTypeComponent c in voxml.Type.Components) {
        			...
        			if (oper == String.Empty) {
        				...
        				if (s.Length > 1) {
        					index = Global.Helper.StringToInt(s[1].Remove(s[1].IndexOf(']')));
        				}
        		...
        		if (str.Length > 1) {
        			int i = Global.Helper.StringToInt(str[1].Remove(str[1].IndexOf(']')));
        		...
        		if (concavity.Length > 1) {
        			int index = Global.Helper.StringToInt(concavity[1].Remove(concavity[1].IndexOf(']')));
        ...
        //		}
        		...
        		foreach (VoxHabitatIntr ih in voxml.Habitat.Intrinsic) {
        			...
        			int index = Global.Helper.StringToInt(s[1].Remove(s[1].IndexOf(']')));
        			//Debug.Log(index);
        		...
        		foreach (VoxAffordAffordance a in voxml.Afford_Str.Affordances) {
        			...
        			int index = (cHabitat.Split('[').Length > 1)
        				? Global.Helper.StringToInt(cHabitat.Split('[')[1].Remove(cHabitat.Split('[')[1].IndexOf(']')))
        				: 0;
