...
namespace VoxSimPlatform {
    namespace Vox {
        ...
        //						Debug.Log (fixHandRotation.rootJoint);
        							...
        							if ((go.tag != "UnPhysic") && (go.tag != "Ground")) {
        								...
        								if (subObj.GetComponent<Rigidbody>() == null) {
        									...
        									if (rigidbody != null) {
        										...
        										float x = Global.Helper.GetObjectWorldSize(subObj).size.x;
        										float y = Global.Helper.GetObjectWorldSize(subObj).size.y;
        										float z = Global.Helper.GetObjectWorldSize(subObj).size.z;
        					...
        					Debug.Log(Global.Helper.VectorToParsable(container.transform.position -
        					                                  Global.Helper.GetObjectWorldSize(container).center));
        				}
        			}
        		}
        		...
        		foreach (GameObject go in allObjects) {
        			if (go.activeInHierarchy) {
        				...
        				if ((Global.Helper.GetMostImmediateParentVoxeme(go).gameObject.transform.parent != null) &&
        					...
        					foreach (Renderer r2 in renderers) {
        						...
        						if (sub1 != sub2) {
        							...
        							if (connectedBody != null) {
        								if ((Global.Helper.GetMostImmediateParentVoxeme(sub1).gameObject.transform.parent == null) &&
        								    (Global.Helper.GetMostImmediateParentVoxeme(connectedBody.gameObject).gameObject.transform
        									     .parent == null)) {
