...
//			reactionTimer.Enabled = false;
			...
			if (objSelector == null) {
				...
				foreach (Voxeme voxeme in objSelector.allVoxemes) {
					...
					if (IsVisible (voxeme.gameObject)) {
						...
						Debug.Log (string.Format ("SyntheticVision.Start:{0}:{1}", voxeme.name, IsVisible (voxeme.gameObject).ToString ()));
					}
				}
			}
		}
		...
		public bool IsVisible(GameObject obj) {
			...
			foreach (Vector3 vertex in vertices) {
				bool hit = Physics.Raycast (vertex, Vector3.Normalize (sensor.transform.position - vertex), 
					Vector3.Magnitude (sensor.transform.position - vertex));
				Debug.Log (string.Format ("SyntheticVision.Update:{0}:{1}:{2}", obj.name, Helper.VectorToParsable(vertex), hit.ToString()));
